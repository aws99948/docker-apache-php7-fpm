...
LABEL maintainer="tecfu <>" \
      ...
      org.label-schema.name="Ubuntu 20.04 with Apache2.4 and PHP 7, optimised using PHP-FPM" \
...
RUN apt-get install -y --allow-downgrades --allow-remove-essential --allow-change-held-packages \
                php7.4-bcmath php7.4-cli php7.4-common php7.4-curl \
                php7.4-mbstring php7.4-mysql php7.4-zip php7.4-fpm
...
RUN echo "error_log = /var/log/php_cli_errors.log" >> /etc/php/7.3/cli/php.ini
